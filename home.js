...
// "Total active time" KPI — today's accumulated sign-in time across ALL
// sessions (not just the current one), fetched fresh from the server
// since sessionStorage only knows about this one sign-in. If a session is
// currently open, its elapsed time is added on top and ticks live.
...
async function loadTotalActiveTime(employeeName) {
    const el = $('statActiveTime'), noteEl = $('statActiveNote');
    try {
        const r = await fetch(`${API_URL}?action=getTodaySummary&employee=${encodeURIComponent(employeeName)}`);
        const result = await r.json();
        if (!(result && result.ok)) return;

        const completedSeconds = Number(result.completedSeconds) || 0;
        const openStart = result.openSince ? new Date(result.openSince).getTime() : null;
        const hasOpenSession = openStart !== null && isFinite(openStart);

        if (activeTimeTimer) clearInterval(activeTimeTimer);
        const tick = () => {
            const liveSeconds = hasOpenSession ? (Date.now() - openStart) / 1000 : 0;
            if (el) el.textContent = formatHoursMinutes(completedSeconds + liveSeconds);
        };
        tick();
        if (noteEl) noteEl.textContent = hasOpenSession ? 'Still counting — you\u2019re signed in' : 'All sessions closed for today';
        if (hasOpenSession) activeTimeTimer = setInterval(tick, 1000);
    } catch (_) {/* leave the placeholder showing */ }
}
...
if (record && record.employee) {
    ...
    loadTotalActiveTime(record.employee);
} else {
    ...
    $('statActiveNote').textContent = 'Sign in to start tracking.';
}
